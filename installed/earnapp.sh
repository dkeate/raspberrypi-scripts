...
find /tmp -maxdepth 1 -type f -name "earnapp_*" | grep -v $VERSION > "$LOG_FILENAME"
echo "$CLEANUP_CMD"
if [ -s $LOG_FILENAME ]; then
    print "✔ Cleaning up..."
    xargs rm -rf -- < "$LOG_FILENAME"
    read_log
...
if [ "$dl_ok" -eq 1 ] && [ -s "$FILENAME" ]; then
    ...
    print "⚠ Failed"
    if ((PRINT_PERR)) && [ -s "$LOG_FILENAME" ]; then
        print "⚠ Fetch log: $(cat "$LOG_FILENAME")"
    fi
    exit 1
fi
...
if [[ "$INSTALLED_VER" == "$VERSION" ]]; then
    ...
    print "⚠ Installation failed (installed='${INSTALLED_VER:-<none>}', expected='${VERSION}')"
    echo
